<script lang="ts">
	...
	async function loadChats() {
		...
		messages.update(() => chats.map(chat => ({ sender: chat.role === 'user' ? 'me' : 'bot', text: chat.content })))
	}
                ...
                if (msg.audio) {
                    ...
                    for (const action of msg.actions) {
                        if (action.name === 'find_and_play_music') {
                            ...
                            lastPlayedAudioPromise = lastPlayedAudioPromise.then(() => playAudio(data.media_url, true));
                        }
                    }
                } else {
					messages.update((msgs) => [
						...msgs,
						{ sender: 'bot', text: msg.text },
					]);
					await tick()
					scrollToBottom();
                }
